...
/*   Updated: 2026/04/06 17:01:01 by bokim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_coders(t_hub *hub)
{
	...
	hub->dongles = malloc(sizeof(t_coder) * hub->config->num_coder);
	if (!hub->coders)
		return 0;
	i = 0;
	while (i < hub->config->num_coder)
	{
		...
		hub->coders[i].config = hub->config;
		hub->coders[i].left_dongle = &hub->dongles[i];
		hub->coders[i].right_dongle = &hub->dongles[(i + 1) % hub->config->num_coder];
		hub->coders[i].last_compile = gettimeofday();
	}
}
